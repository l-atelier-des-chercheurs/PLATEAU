<template>
  <form @submit.prevent="newChat" class="formbox margin-sides-medium">
    <!-- <Modal
    @close="$emit('close')"
    :read_only="read_only"
  ...
  > -->
    ...
    <div class="margin-bottom-small" v-if="false">
  ...
  data() {
    return {
      ...
      chatdata: {
        name: "",
        pinned: false,
        authors: this.$root.current_project.authors,
        editing_limited_to: this.$root.current_project.editing_limited_to,
        viewing_limited_to: this.$root.current_project.viewing_limited_to,
        linked_project: this.$root.current_project.slugFolderName,
      },
